...
const Reviews = () => {
  ...
  useEffect(() => {
    if (title1.current) {
      const split = new SplitType(title1.current, {
        types: 'chars,words',
      });
  ...
  return (
    <div className=" w-full px-10 py-20 max-sm:p-5 overflow-hidden">
      ...
      {navigationReady && (
        <div ref={reviewsContainer} className='max-w-[1700px] mx-auto 
        '>
          <Swiper
            slidesPerView={1}
            spaceBetween={10}
          ...
          >
            {reviews.map((review, index) => (
              <SwiperSlide key={index}>
                <div className='p-5 gap-3 w-full  flex flex-col  h-full justify-between bg-[#0C0B0E] border-[#151518] border-[1.5px] rounded-xl '>
